...
Given an array of integers, find the peak index. A peak index is defined as the position `i` such that:

- The elements before index `i` are strictly increasing.
- The elements after index `i` are strictly decreasing.

The array is guaranteed to have exactly one peak element. If the array size is less than 3, return -1.
...
- **Time Complexity**: O(n)
- **Notes**: Works fine for small inputs, but inefficient when input size grows.
